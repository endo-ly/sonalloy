...
def main() -> None:
    additive_source_path = (
        ROOT / "testdata" / "instruments" / "additive-generator-reference.json"
    )
    additive_source = json.loads(additive_source_path.read_text(encoding="utf-8"))
    source_path = ROOT / "testdata" / "instruments" / "formant-generator-reference.json"
    source = json.loads(source_path.read_text(encoding="utf-8"))
    hybrid_source_path = (
        ROOT / "testdata" / "instruments" / "harmonic-formant-hybrid-reference.json"
    )
    review_root = ROOT / "review" / "harmonic-formant-synthesis"
    definition_dir = review_root / "definitions"
    event_dir = review_root / "events"
    ...
    existing_review_definitions = [
        (ROOT / "testdata" / "instruments" / "basic-poly-synth.json"),
        (ROOT / "testdata" / "instruments" / "moving-hybrid-pad.json"),
        (ROOT / "testdata" / "instruments" / "processed-hybrid.json"),
        (ROOT / "review" / "generate" / "fixtures" / "essential-hybrid-instrument.json"),
        (ROOT / "review" / "generate" / "fixtures" / "granular-generator-reference.json"),
        (ROOT / "review" / "generate" / "fixtures" / "wave-sequence-hybrid-reference.json"),
        (ROOT / "review" / "generate" / "fixtures" / "digital-hybrid-reference.json"),
    ]
    ...
    existing_render_jobs = [
        (
            "27-existing-processor-chain.wav",
            ROOT / "testdata" / "instruments" / "processed-hybrid.json",
        ),
        (
            "28-existing-digital-hybrid.wav",
            ROOT / "review" / "generate" / "fixtures" / "digital-hybrid-reference.json",
        ),
    ]
    existing_render_paths: dict[str, Path] = {}
    for audio_name, definition_path in existing_render_jobs:
        path = technical_dir / audio_name
        render_note(
            definition_path,
            60,
        ...
        existing_render_paths[definition_path.name] = path
        append_path(generated_paths, path)
