...
def main() -> None:
    subprocess.run(
        [
            sys.executable,
            str(ROOT / "testdata" / "generate" / "generate_metallic_hybrid_inputs.py"),
        ],
        cwd=ROOT,
        check=True,
    )

    review_root = ROOT / "review" / "metallic-hybrid"
    ...
    definition_sources = {
        "metallic-hybrid.json": ROOT / "testdata" / "instruments" / "metallic-hybrid.json",
