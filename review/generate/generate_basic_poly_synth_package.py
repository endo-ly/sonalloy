...
def main() -> None:
    subprocess.run(
        [
            sys.executable,
            str(ROOT / "testdata" / "generate" / "generate_midi_fixtures.py"),
        ],
        check=True,
    )
    review_root = ROOT / "review" / "basic-poly-synth"
    ...
    subprocess.run(
        [
            sys.executable,
            str(ROOT / "review" / "generate" / "generate_basic_poly_synth_metrics.py"),
        ],
        check=True,
    )
