...
impl FlangerRuntime {
    pub(crate) fn new(compiled: &CompiledFlangerProcessor) -> Self {
        Self {
    ...
    pub(crate) fn process(
        ...
        for index in 0..left.len() {
            ...
            let left_delay = self.delay_frames * (1.0 + 0.95 * depth * left_lfo);
            let right_delay = self.delay_frames * (1.0 + 0.95 * depth * right_lfo);
